...
function showData(array) {
  array.forEach((el) => {  // 'el' represents each object in the array
      ...
      h2.innerHTML = el.name;  // 'el.name' will contain the name of the product

      // Create and add the category
      let p1 = document.createElement("p");
      p1.innerHTML = el.category;  // 'el.category' will contain the category of the product

      // Create and add the price
      let p2 = document.createElement("p");
      p2.innerHTML = `$${el.price}`;  // '$' symbol added for price display

      // Create and add the rating
      let p3 = document.createElement("p");
      p3.innerHTML = `Rating: ${el.rating} / 5`;  // Adding a more readable rating display

      // Create and add the image
      let img = document.createElement("img");
      img.src = el.image;  // 'el.image' will contain the image URL
      img.style.height = "150px";  // Adjusted for fixed size
      img.style.width = "150px";   // Adjusted for fixed size
      ...
      // Append the box to the parent container (products div)
      document.getElementById("products").append(box);  // Ensure 'products' exists in your HTML
  });
}
